...
// Armar clasificacion: "2D - NT", "4D - VQ", etc.
...
export async function GET(request: NextRequest) {
  try {
    ...
    const camaraGlobal = searchParams.get('camara') || '01' // default: camara 01
    ...
    // camaraPorTropa: JSON string "{"165":"02","166":"03"}" (opcional, sobreescribe camaraGlobal)
    const camaraPorTropaStr = searchParams.get('camaraPorTropa')
    const camaraPorTropa: Record<string, string> = camaraPorTropaStr ? JSON.parse(camaraPorTropaStr) : {}
    ...
    // Helper: obtener camara para una tropa
    const getCamara = (tropaNum: number) => {
      return camaraPorTropa[String(tropaNum)] || camaraGlobal
    }
    ...
    // Sin header - SIGICA importa sin encabezado
    ...
    for (const rom of romaneos) {
      ...
      const tropaCamara = getCamara(tropaNum)

      if (rom.mediasRes.length === 0) {
        // Sin medias res: usar pesoMediaIzq y pesoMediaDer del romaneo
        if (rom.pesoMediaIzq) {
          filas.push([String(tropaNum), especie, fechaStr, periodo, clasificacion, despiece, String(rom.garron), String(rom.pesoMediaIzq), tropaCamara, tropaDestino, '', '', ''])
        }
        if (rom.pesoMediaDer) {
          filas.push([String(tropaNum), especie, fechaStr, periodo, clasificacion, despiece, String(rom.garron), String(rom.pesoMediaDer), tropaCamara, tropaDestino, '', '', ''])
        }
      } else {
        // Con medias res: usar los datos de cada media, con camara seleccionada
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fecha, destino, camara, tropaIds, destinoPorTropa, camaraPorTropa } = body
    ...
    const tropasConResumen = await Promise.all(tropas.map(async (tropa) => {
      ...
      const tropaCamara = (camaraPorTropa && camaraPorTropa[String(tropa.numero)]) || camara || '01'
