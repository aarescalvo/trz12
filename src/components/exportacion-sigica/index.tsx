...
export function ExportacionSIGICAModule({ operador }: { operador: Operador }) {
  ...
  const [camaraGlobal, setCamaraGlobal] = useState('01')
  ...
  // Cámara individual por tropa (sobreescribe la cámara global)
  const [camaraPorTropa, setCamaraPorTropa] = useState<Record<number, string>>({})
  ...
  // Cambiar cámara individual por tropa
  const cambiarCamaraTropa = (tropaNum: number, nuevaCamara: string) => {
    setCamaraPorTropa(prev => {
      const updated = { ...prev, [tropaNum]: nuevaCamara }
      return updated
    })
  }
  ...
  // Filtrar registros por tropas seleccionadas y aplicar destino y cámara por tropa
  const registrosFiltrados = useMemo(() => {
    ...
    return preview.registros
      ...
      .map(r => ({
        ...
        camara: camaraPorTropa[r.tropa] || camaraGlobal,
      }))
  }, [preview, tropasSeleccionadas, destinoPorTropa, destinoGlobal, camaraPorTropa, camaraGlobal])
  ...
  const descargarCSV = async () => {
    ...
    try {
      ...
      // Pasar cámara por tropa como JSON
      if (Object.keys(camaraPorTropa).length > 0) {
        params.set('camaraPorTropa', JSON.stringify(camaraPorTropa))
      }
          ...
          <CardContent className="pt-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={camaraGlobal} onValueChange={setCamaraGlobal}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    {preview.tropas.map((tropa) => {
                      ...
                      return (
                        <TableRow key={tropa.tropaId} className={tropasSeleccionadas.includes(tropa.tropaNumero) ? 'bg-green-50/50' : ''}>
                          ...
                          <TableCell className="text-center">
                            <Select
                              value={camaraPorTropa[tropa.tropaNumero] || camaraGlobal}
                              onValueChange={(val) => cambiarCamaraTropa(tropa.tropaNumero, val)}
                            >
